...
#TODO Infinite Deck 
# This draws card from an infinite deck, for card counting to work we want there to be a fixed number of decks
# Also we want all the player's cards to be displayed, not just the sum of their cards, again for card counting purposes
...
#TODO fix the action_space, so that only legal actions are available

        self.action_space = spaces.Discrete(3) if self.step_n == 0 else spaces.Discrete(2)
    ...
    def step(self, action):
        ...
        #TODO implement the split operation
    ...
    def render(self):
        ...
        dealer_text = small_font.render(
            "Mr. White: " + str(dealer_card_value), True, white
        )
...
# We won't be considering "double down" since we aren't taking the profits into consideration

def get_legal_actions(obs):
    legal_actions = ["hit", "stick"]
    #if the two cards that are in the players hands are equal, we can split
    if self.player[0] == self.player[1] and self.step_n == 0:
